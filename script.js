...
class fetchForecastApi {
    ...
    // to bypass Same Origin Policy (SOP) error
    addCorsHeader() {
        // to pre-filter every request that getJSON does
    ...
    getLocation(query, callback) {
        // passing data we got from the api to fetchWeather
        $.getJSON(this.searchApiUrl, { query }).done(data => callback(data)).fail(() => callback(null)); // because of es6 syntax { query } = { query: query } 
    }
...
// class for storing all the elements we have in html in their respective variables
...
class dataMiddleware {
    ...
    prepareDataForDom(data) {
        ...
        const todayForecastDetails = this.gatherTodaysForecastDetails({
            predictability, 
            humidity, 
            wind_speed, 
            air_pressure, 
        ...
        // prepare goes before ui changes to put data in the ui but not display it yet
        this.displayForecast.showTodayForecast(todaysForecastGeneral)
        this.prepareTodayForecastDetails(todayForecastDetails);
    ...
    // change the data we are receiving in order for our ui to look as it should
    prepareTodayForecastDetails(forecast) {
...
// main source of input interaction with the user
class requestController {
    ...
    // gives us the query from search input field
    getQuery() {
        return $('#search-query').val().trim();  // added trim to trim all the white spaces
    }

    fetchWeather(query) {
        // send query to getLocation
        this.fetchForecastApi.getLocation(query, location => {

            if (!location || location.length === 0) {
                this.coreDomElements.showError('We could not find this location, please try again.');
                return;
            }
            ...
            this.fetchForecastApi.getWeatherData(location[0].woeid, data => {
                ...
                if (!data) {
                    this.coreDomElements.showError('We could not proceed with the request, please try again later.');
                    return;
                }
